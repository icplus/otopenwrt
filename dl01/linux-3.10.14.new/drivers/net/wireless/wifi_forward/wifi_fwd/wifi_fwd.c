...
void wifi_fwd_feedback_map_table(void *adapter, void **peer, void **opp_peer)
{
	...
	if ((adapter == adapter_2g) && (global_map_5g_tbl.Enabled == TRUE)) {
		*peer = &global_map_2g_tbl;
		*opp_peer = &global_map_5g_tbl;
		
	} else if ((adapter == adapter_5g) && (global_map_2g_tbl.Enabled == TRUE)) {
...
static bool is_gratuitous_arp(unsigned char *pData)
{
	...
	ProtoType = ntohs(ProtoType);
	Pos += 2;	

	if (ProtoType == ETH_TYPE_ARP)
	{
		/* 
		...
		if (memcmp(SenderIP, TargetIP, 4) == 0) {
			return TRUE;
		}
	}
	return FALSE;
...
static bool is_dad_packet(void *adapter, unsigned char *pData)
{
	void *opp_band_tbl =NULL;
	void *band_tbl =NULL;
	unsigned char *pSenderIP = pData + 16;
	unsigned char *pSourceMac = pData + 10;
	unsigned char *pDestMac = pData + 20;
	unsigned char ZERO_IP_ADDR[4] = {0x00, 0x00, 0x00, 0x00};
	...
	* Check if DAD packet
	*/
	wifi_fwd_feedback_map_table(adapter, &band_tbl, &opp_band_tbl);

	if (band_tbl == NULL)
		return FALSE;
	
	if ((RTMPQueryLookupRepeaterCliEntryMT(band_tbl, pSourceMac, FALSE) == TRUE) &&
		((MAC_ADDR_EQUAL(pDestMac, BROADCAST_ADDR) == TRUE) ||
		(MAC_ADDR_EQUAL(pDestMac, ZERO_MAC_ADDR) == TRUE)) &&
		(!memcmp(pSenderIP, ZERO_IP_ADDR, 4) == TRUE)) {
...
static bool wifi_fwd_tx_check_hook(void *adapter, struct net_device *tx_dev, struct sk_buff *skb)
{
	unsigned char *pSrcBuf = &skb->data[14];
	unsigned short TypeLen = 0;
 	
	if(!is_wifi_concurrent_mode(tx_dev))
		return TRUE;

	TypeLen = (((unsigned char *)skb->data)[12] << 8) | ((unsigned char *)skb->data)[13];

	if (TypeLen != ETH_TYPE_ARP)
		return TRUE;

	if (is_dad_packet(adapter, pSrcBuf - 2) || is_gratuitous_arp(pSrcBuf - 2))
		return FALSE;
...
static int wifi_fwd_tx_handler(struct sk_buff *skb)
{
	...
	*/
	if (!WIFI_FWD_TEST_FLAG(fOP_WIFI_FWD_ACTIVE) || !skb || (fwd_counter == 0))
		return 0;
...
static int wifi_fwd_init_mod(void)
{
	...
	wf_fwd_tx_check_hook = wifi_fwd_tx_check_hook;
